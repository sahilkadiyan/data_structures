...
class Polynomial {
   ...
   //default constructor
    ...
    void setCoefficient(int degree,int coefficient){
        if(degree>capacity){
            int newcapacity=degree;    //so that newcapacity will now serve the degree 
            int *newdeg=new int[newcapacity+1];   //+1 is for length beacuse degree starts from 0 index
